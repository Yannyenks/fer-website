  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Filtrer uniquement les candidats de type "miss"
        const missCandidates = list.filter(c => (c.type || 'miss') === 'miss');
        if (mounted) {
          setCandidates(missCandidates);
          setLoading(false);
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-[#0c0c0c] text-white px-6 py-16">
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="mt-4 text-gray-400">Chargement des candidats...</p>
        </div>
      ) : candidates.length === 0 ? (
        <div className="text-center py-20">
          <div className="mb-6">
          ...
          <h2 className="text-2xl font-semibold mb-3 text-gray-300">Aucun candidat pour le moment</h2>
          <p className="text-gray-500">Les candidats seront bientôt disponibles. Revenez plus tard !</p>
        </div>
      ) : (
